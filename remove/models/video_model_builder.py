...
from pyaction.models import head_helper, resnet_helper, stem_helper
...
class MetaResNetModel(nn.Module):
    """
    ResNet model builder. It builds a ResNet like network backbone without
    lateral connection (C2D, I3D, SlowOnly).
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode): model building configs, details are in the
                comments of the config file.
        """
        super(MetaResNetModel, self).__init__()
        self.enable_detection = cfg.DETECTION.ENABLE
        self.num_pathways = 1
        self._construct_network(cfg)
        init_helper.init_weights(
    ...
    def _construct_network(self, cfg):
        ...
        if self.enable_detection:
            ...
            #     dropout_rate=cfg.MODEL.DROPOUT_RATE,
            # )
            self.head = head_helper.MetaClsHead(
                cfg,
                pool_size=pool_size
            )

    def forward(self, x, support_label, bboxes=None):
        x = torch.cat(x, dim=1)
        B, S, _, T, H, W = x.shape
