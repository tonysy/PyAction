...
def convert_one_hot(labels, N_way):
    """
    Convert the int label into its one-hot format
    Args:
    ...
    onehot_labels.scatter_(2, labels.unsqueeze(-1), 1.0)
...
class CosSimMetric(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.META.MODEL.COSINE_SCALE_LEARN:
            self.scale = nn.Parameter(torch.ones(1) * cfg.META.MODEL.COSINE_SCALE)
        else:
            self.scale = cfg.META.MODEL.COSINE_SCALE
