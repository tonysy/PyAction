...
with open("tools/pyaction_meta_train", "w") as pyaction_meta_train:
    head = f"#!/bin/bash\n\nexport OMP_NUM_THREADS=1\n"
    pyaction_meta_train.write(
        head + f"python3 {os.path.join(cur_dir, 'tools', 'meta_train.py')} $@"
    )

with open("tools/pyaction_meta_test", "w") as pyaction_meta_test:
    head = f"#!/bin/bash\n\nexport OMP_NUM_THREADS=1\n"
    pyaction_meta_test.write(
        head + f"python3 {os.path.join(cur_dir, 'tools', 'meta_test.py')} $@"
    )
...
setup(
    ...
    scripts=["tools/pyaction_run", "tools/pyaction_meta_train", "tools/pyaction_meta_test"],
)
